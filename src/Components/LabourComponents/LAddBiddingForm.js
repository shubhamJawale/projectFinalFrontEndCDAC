...
export default function LAddBidding() {
    ...
    let addwork = () => {
        ...
        axios.post(`${baseurl}/labour/addbidiingbylabour/${contId}/${workId}`, bidding).then((response) => {
            ...
            setAmount("");
            settdate("");
            history.push("/labour/showBidding");
    ...
    let amtregex = /(?=.*\d)^\$?(([1-9]\d{0,2}(,\d{3})*)|0)?(\.\d{1,2})?$/;
    ...
    return (
        <div className="col-6 offset-3 justify-content-center justify-content-center my-5 bg bg-info">
            <form className="form-control bg-info">
                <h4 className="labels">Add Bidding</h4>
                ...
                <br />
                <input type="button" onClick={validate} value="Add Bidding" className="btn btn-warning form-control" />
            </form>
        </div>)
}
