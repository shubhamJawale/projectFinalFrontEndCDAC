...
export default function AssignedWorkListForContractor() {
    ...
    let getAssignedWorks = (id) => {
        axios.get(`${baseurl}/contractor/getalldata/AssignedWork/${id}`).then((response) => {
        ...
        <div>
            <ToastContainer />
            {
                assignworklist.map((workset) => {
                    return (
                        (workset.astatus === "COMPLETED") ? <Card key="{workset.assignedWorkId}" color="success" inverse className="m-1 justify-content-center text-center">
                            <CardHeader>
